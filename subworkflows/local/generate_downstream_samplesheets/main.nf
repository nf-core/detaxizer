    ...
    ch_list_for_samplesheet = ch_reads_grouped
                                .map {
                                    meta, reads ->
                                        ...
                                        def short_reads_1  = meta.long_reads == (reads.size() > 2) ? reads[0].getName() : ""                    // If long reads, but no short reads, then short_reads_1 is empty
                                        def short_reads_2  = meta.long_reads == (reads.size() > 2) && reads[1] ? reads[1].getName() : ""
                                        def long_reads     = meta.long_reads ? reads.last().getName() : ""                                      // If long reads, take final element
